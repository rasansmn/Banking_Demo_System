...
Public Class frmWithdrawal
    ...
    Dim intCounter, intCounter2, intTranscounter, intNewtransid, intMytransactions As Integer
    ...
    Sub loadgrid()
        Try
            idmdb.Open()
            Dim dadapt As New SqlClient.SqlDataAdapter("SELECT * FROM transactions WHERE trans_type ='Withdrawal' ORDER BY trans_id DESC", idmdb.objcon)
    ...
    Private Sub btnSubmit_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSubmit.Click
        ...
        If txtAccnum.Text <> "" And txtNic.Text <> "" Then
            Try
                idmdb.Open()
                ...
                Dim cmd2 As New SqlCommand("SELECT COUNT (*) from account WHERE nic_num = '" & txtNic.Text & "'", idmdb.objcon)
                intCounter2 = cmd2.ExecuteScalar
                Dim cmd3 As New SqlCommand("SELECT acc_balance from account where acc_num = '" & Val(txtAccnum.Text) & "'", idmdb.objcon)
                balance = cmd3.ExecuteScalar
        ...
        If Trim(txtAccnum.Text) = "" Then
            ...
            txtAccnum.Focus()
        ElseIf intCounter = 0 Or IsNumeric(txtAccnum.Text) = False Then
        ...
        ElseIf Trim(txtNic.Text) = "" Then
            MessageBox.Show("Please fill the NIC number", "Submit", MessageBoxButtons.OK, MessageBoxIcon.Error)
            txtNic.Focus()
        ...
        ElseIf IsNumeric(txtRs.Text) = False Or IsNumeric(txtCts.Text) = False Then
            MessageBox.Show("Please fill the amount", "Submit", MessageBoxButtons.OK, MessageBoxIcon.Error)
            txtRs.Focus()
        ElseIf intCounter2 = 0 Then
            MessageBox.Show("This NIC number is not valid with this account number", "Submit", MessageBoxButtons.OK, MessageBoxIcon.Error)
            txtNic.Focus()
        ElseIf balance < amount Then
            MessageBox.Show("No sufficient account balance for this transaction", "Submit", MessageBoxButtons.OK, MessageBoxIcon.Error)
            txtRs.Focus()
        Else
            newbalance = balance - amount
            Dim curtime As String = DateTime.Now.ToString
            Dim result As DialogResult = MessageBox.Show("You are going to do a new transaction" & vbCrLf & vbCrLf & "Account number: " & txtAccnum.Text & vbCrLf & "Current balance: Rs." & balance & vbCrLf & "Transaction amount: Rs." & amount & vbCrLf & "New balance will be: Rs." & newbalance & vbCrLf & vbCrLf & "Proceed this transaction?", "New Withdrawal", MessageBoxButtons.YesNo)
            If result = Windows.Forms.DialogResult.Yes Then
                Try
                    ...
                    Dim cmdin As New SqlCommand("INSERT INTO transactions VALUES (" & intNewtransid & "," & txtAccnum.Text & ",'" & DateTime.Now & "','Withdrawal','" & txtRs.Text & "','" & newbalance & "','" & txtName.Text & "','" & strName & "','" & intAccnum & "')", idmdb.objcon)
                    ...
                    MessageBox.Show("Records updated successfully", "New Withdrawal", MessageBoxButtons.OK, MessageBoxIcon.Information)
                    clear()
                Catch ex As Exception
                    MessageBox.Show(ex.Message, "Database Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
                Finally
